...
# **Part 5: Advanced Leaflet Features**

In the previous exercises, we learned how to display raster and vector
data in an interactive map. The following exercises introduce additional
Leaflet features that are commonly used in web GIS applications and
Shiny dashboards.

### **Learning objectives**

- Add interactive popups and labels
- Organize layers using layer controls
- Explore alternative basemaps
- Compare observations with model predictions
- Build a more complete interactive mapping application

## **Exercise 1: Add Popups to Observation Points**

Popups allow users to click on a feature and retrieve information about
that location.

Modify the point layer so that clicking a point displays information
...
- What information is most useful in a popup?
- How is a popup different from a label?

## **Exercise 2: Add Hover Labels**

Labels appear when the cursor hovers over a feature.

```{r, echo=TRUE}
addCircleMarkers(
  data = lilacObs_sf,
  label = ~paste("Observed month:", leaf_month)
)
```

Try displaying different variables from the observation dataset.

## **Exercise 3: Layer Controls**

Many web maps contain multiple datasets that users can toggle on and
off.
...
- Why might users want to hide layers?
- Which layer is easier to interpret on its own?

## **Exercise 4: Compare Basemaps**

Leaflet supports many different basemap providers.
...
Compare the advantages and disadvantages of each basemap.

## **Exercise 5: Add a Map Title**

Map titles can be added using custom HTML controls.

```{r, echo=TRUE}
leaflet() %>%
  addControl(
    html = HTML(
      "<h4>Lilac Leaf-Out Predictions (2018)</h4>"
    ),
    position = "topleft"
  )
```

Try modifying the title and adding additional information.

## **Exercise 6: Prediction Error Map**

One useful application is comparing observations against model
predictions.
...
lilacObs_sf$error <- predicted_values -
  lilacObs_sf$leaf_doy
```

Create a palette for prediction error.

```{r, echo=TRUE}
palError <- colorNumeric(
  palette = "RdBu",
  domain = lilacObs_sf$error
)
```
...
- Does the model tend to predict earlier or later leaf-out dates?
...
The final product should function as a small web GIS application.
