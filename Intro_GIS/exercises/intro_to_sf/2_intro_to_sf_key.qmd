...
subtitle: "Working with Vector Data, Part 2"
date: today
author: "Roger Andre"
...
*Note: When creating the "geometry" column, the source columns will be removed and converted.*
...
*NOTE: `st_distance` will automatically calculate great circle distances when it detects that lat, lon coordinates are used.*

------------------------------------------------------------------------

#### 2. **How many reservoirs are within 55 km of Sprague, WA?**
...
*Something else to remember is that `st_intersection` is generally slower than `st_intersects`, since it has to do more work to actually process geometry.*

------------------------------------------------------------------------

### **Practice 1. Create sf features from coordinates. Calculate the distance between Paris, FR and Moscow RU.**
...
st_distance(cities_sf[1,], cities_sf[2,]) %>% 
  units::set_units("km")
```

------------------------------------------------------------------------

### **Practice 2. Create a 2500km buffer around Paris. Plot both the buffer geometry and the city points. Label them. (Don't forget to reproject)**

```{r, echo=TRUE, message=FALSE}
# Reproject cities_sf to ESRI:102013
cities_albers <- st_transform(cities_sf, crs = 'ESRI:102013')

# Create a 2500km buffer around Paris
paris_buffer <- st_buffer(cities_albers[1,], (2500 * 1000))

# Plot
plot(paris_buffer$geometry)
plot(cities_albers$geometry, pch=20, add = TRUE)
text(st_coordinates(cities_albers), pos=1, cities_albers$city, cex=0.75)
```

------------------------------------------------------------------------

### **Practice 3. Prove that that Moscow is within the paris_buffer using `st_intersects`. Prove that Yekaterinburg is not.**

```{r, echo=TRUE, message=FALSE}
st_intersects(paris_buffer$geometry, cities_albers, sparse = FALSE)
```
