...
**Question 1.** How does predicted lilac leaf out vary across the
contiguous U.S. (CONUS)? Any ideas what may explain these patterns?
...
**Challenge 1.** Change at least two plot features to align with your
preferences. For example, you could remove the plot axes (lat and long),
change the legend attributes, font types and sizes, etc. (`my_theme`),
or change the color palette (`pal`).
...
boundaries to the same extent as our raster data. See
[here](https://r.geocompx.org/reproj-geo-data) for more information
about the S2 spherical geometry engine.
...
**Question 2.** What is the class and geometry type of `states`?
Describe two other differences between `states` and `lilacModel_r`.
...
**Challenge 2.** Can you think of a `dplyr` approach to obtaining data
only for CONUS? Hint: remove rows with undesired states (note that
Puerto Rico \[PR\] is in the data).
...
**Question 3.** What happens when you don't enter `NA` for the fill
option for the `states_c` feature? Why?
...
makes it save as an ESRI Shapefile. Note: GDAL requires that column
...
**Question 4.** How does the geometry of the lilac observations
(`lilacObs_sf`) differ from the states feature? Why?
...
information and colored according to values in the `leaf_date` column.
...
**Question 5.** What does your map tell you about predicted vs. observed
first leaf out in lilac? Can you think of a better way to compare the
two datasets on a map?
...
```{r, Inteecho=TRUE, warnings=FALSE}
# Define your limits
...
Mapping](https://collegepublishing.sagepub.com/products/an-introduction-to-r-for-spatial-analysis-and-mapping-3-288946)
by C. Brunsdon and L. Comber (2025)\
- [Geocomputation with R](https://r.geocompx.org/) by R. Lovelace et al.
(2025)\
- [Data Analysis and Visualization with R:
Spatial](https://sunsp.net/rspatial_book/) by S. Sun (2025)\
...
- [R as GIS for
Economists](https://tmieno2.github.io/R-as-GIS-for-Economists/) by Taro
Mieno (2024)\
...
`rnpn`.Development of this workshop was supported by the USDA National
